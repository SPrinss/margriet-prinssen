...
class RecensiesPage extends MPElement {
  ...
  async getRecensieById(id) {
    ...
    const resp = await fetch(
      `https://firestore.googleapis.com/v1/projects/margriet-prinssen/databases/(default)/documents/reviews/${id}`
    );
    ...
    return html`
      ...
      <mp-page ?active="${!this.recensieId}">
        ...
        <section id="intro-section">
          <p>
            Zoeken is mogelijk op titels, gezelschappen, personen, theaters,
            jaartallen en steden. Als je binnen een categorie (bijvoorbeeld
            Toneelschuur) wilt zoeken, kan je op ‘Gebruik als filter’ klikken.
          </p>
        </section>
      ...
      <mp-recensie
        ?active="${this.recensieId}"
        .authToken=${this.authToken}
        .recensieId=${this.recensieId}
